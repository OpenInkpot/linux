...
static struct fb_fix_screeninfo apollofb_fix = {
	.id =		"apollofb",
...
static struct fb_var_screeninfo apollofb_var = {
	.xres		= DPY_W,
	.yres		= DPY_H,
	.xres_virtual	= DPY_W,
	.yres_virtual	= DPY_H,
...
static int apollo_send_data_fast(void *arg, unsigned char data)
{
	struct apollofb_par *par = arg;
	int res = 0;
		...
		int last_fragment)
{
	...
	int(*func_send_data_fast)(void*, unsigned char);
				...
				size_t count, loff_t *f_pos)
{
	...
	printk(KERN_INFO "*f_pos = %lld, count = %u\n", *f_pos, count);
...
static int apollofb_setup_chrdev(struct apollofb_par *par)
{
	int res = 0;
...
static int apollofb_probe(struct platform_device *dev)
{
	struct fb_info *info;
	int retval = -ENOMEM;
	int videomemorysize;
...
static int apollofb_remove(struct platform_device *dev)
{
